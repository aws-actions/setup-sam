...
async function uninstallExistingWindowsSamCli(nightly) {
  ...
  // Query the uninstall registry for products whose InstallLocation matches
  // our target root, then run `msiexec /x <ProductCode>` for each match.
...
$entries = Get-ItemProperty $paths -ErrorAction SilentlyContinue | Where-Object {
  $_.InstallLocation -and ($_.InstallLocation.TrimEnd('\\') -ieq $root)
}
